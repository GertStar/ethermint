...
>e.g. `gert/gert-sim:version1`, `go test ./app`

This can run sims but doesn't collect the results. This is handled by a custom script.

## Running sims and uploading to S3

The dockerfile in this repo defines the docker image to run sims. It includes the gert source code, aws cli, and the custom simulation script.
...
- create and submit a new array job (based of the job definition) with
  - image `gert/gert-sim:<some-version>`
...
- any changes needed to the code or script necessitates a rebuild:
  - `docker build -f simulations/Dockerfile -t gert/gert-sim:<some-version> .`
  - `docker push gert/gert-sim:<some-version>`
