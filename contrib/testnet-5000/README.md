...
Testnet-5000 introduces transfers between gert and Bnbchain via BEP3.

This guide will walk you through interacting with the blockchains and transferring coins via the rest server. To send transactions, we'll create an unsigned request, sign it, and broadcast it to the gert blockchain.

## Rest server requests

### Setup

We'll be using gert's CLI to build, sign, and broadcast the transactions:
...
Congratulations, you've just created a swap on gert! The swap will be automatically relayed over to Bnbchain where it it can be claimed using the secret random number from above.
