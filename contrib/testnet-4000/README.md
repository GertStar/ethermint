...
Resources and examples for running and interacting with gert-testnet-4000
...
If testing locally, start the gert rest server:

```bash
    kvcli rest-server
```

Now we'll create an unsigned request, sign it, and broadcast it to the gert blockchain via the rest server. Note that if you're using the mainnet or testnet, the host IP address will need to be updated to point at an active rest server instead of http://127.0.0.1.
...
Congratulations, you've just created a CDP on gert using the rest server!

### Post market price example request

Note that only market oracles can post prices, other senders will have their transactions rejected by gert.
...
Congratulations, you've just posted a current market price on gert using the rest server!
...
To run the dredd tests to hit the endpoints and test the api run the following command from the `gert` folder:
