...
# gert1ffv7nhd3z6sych2qpqkk03ec6hzkmufy0r2s4c
...
# gert1adkm6svtzjsxxvg7g6rshg6kj9qwej8gwqadqd
...
# gert18jz5lyhhy6ncjlyty064kttw93yzaulq7rlptu
...
# gert10wlnqzyss4accfqmyxwx5jy5x9nfkwh6qm7n4t
...
# gert15tmj37vh7ch504px9fcfglmvx6y9m70646ev8t
...
# Set client chain id
sed -in-place='' 's/chain-id = ""/chain-id = "gertlocalnet_8888-1"/g' $DATA/config/client.toml
...
$BINARY add-genesis-account $validatorKeyName 2000000000ugert,100000000000bnb
...
$BINARY add-genesis-account $faucetKeyName 1000000000ugert,100000000000bnb

evmFaucetKeyName="evm-faucet"
printf "$evmFaucetMnemonic\n" | $BINARY keys add $evmFaucetKeyName --eth --recover
$BINARY add-genesis-account $evmFaucetKeyName 1000000000ugert

userKeyName="user"
printf "$userMnemonic\n" | $BINARY keys add $userKeyName --eth --recover
$BINARY add-genesis-account $userKeyName 1000000000ugert

relayerKeyName="relayer"
printf "$relayerMnemonic\n" | $BINARY keys add $relayerKeyName --eth --recover
$BINARY add-genesis-account $relayerKeyName 1000000000ugert

# Create a delegation tx for the validator and add to genesis
$BINARY gentx $validatorKeyName 1000000000ugert --keyring-backend test --chain-id $chainID
$BINARY collect-gentxs

# Replace stake with ugert
sed -in-place='' 's/stake/ugert/g' $DATA/config/genesis.json

# Replace the default evm denom of aphoton with ugert
sed -in-place='' 's/aphoton/agert/g' $DATA/config/genesis.json
...
jq '.app_state.bridge.params.relayer = "gert15tmj37vh7ch504px9fcfglmvx6y9m70646ev8t"' $DATA/config/genesis.json | sponge $DATA/config/genesis.json
...
jq '.app_state.bridge.params.enabled_conversion_pairs = [
    {
        gert_erc20_address: "0x404F9466d758eA33eA84CeBE9E444b06533b369e",
        denom: "erc20/weth",
    }]' $DATA/config/genesis.json | sponge $DATA/config/genesis.json
