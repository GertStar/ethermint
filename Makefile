...
HTTPS_GIT := https://github.com/Gert-Chain/core.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=gert \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=gert \
...
build: go.sum
ifeq ($(OS), Windows_NT)
	go build -mod=readonly $(BUILD_FLAGS) -o build/$(shell go env GOOS)/gert.exe ./cmd/gert
else
	go build -mod=readonly $(BUILD_FLAGS) -o build/$(shell go env GOOS)/gert ./cmd/gert
endif
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/gert
...
link-check:
	...
	liche -r . --exclude "^http://127.*|^https://riot.im/app*|^http://gert-testnet*|^https://testnet-dex*|^https://gert3.data.gert.io*|^https://ipfs.io*|^https://apps.apple.com*|^https://gert.quicksync.io*"
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' | xargs goimports -w -local github.com/Gert-Chain/core
.PHONY: format
...
build-docker-local-gert:
	@$(MAKE) -C networks/local

# Run a 4-node testnet locally
localnet-start: build-linux localnet-stop
	@if ! [ -f build/node0/kvd/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/kvd:Z gert/gertnode testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; fi
	docker-compose up -d
...
start-remote-sims:
	...
	docker build -f simulations/Dockerfile -t gert/gert-sim:master .
	# push that image to the hub
	docker push gert/gert-sim:master
	...
	aws batch submit-job \
		...
		-—job-definition gert-sim-master \
