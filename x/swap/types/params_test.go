...
func TestParams_Validation(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			testFn: func(params *types.Params) {
				params.AllowedPools = types.NewAllowedPools(types.NewAllowedPool("ugert", "ugert"))
			},
			expectedErr: "pool cannot have two tokens of the same type, received 'ugert' and 'ugert'",
		},
		{
			name: "nil swap fee",
...
func TestParams_String(t *testing.T) {
	...
	assert.Contains(t, output, types.PoolID("hard", "ugert"))
	assert.Contains(t, output, types.PoolID("ugert", "usdx"))
...
func TestAllowedPool_Validation(t *testing.T) {
	testCases := []struct {
		...
		{
			name:        "blank token a",
			allowedPool: types.NewAllowedPool("", "ugert"),
		...
		},
		{
			...
			allowedPool: types.NewAllowedPool("1ugert", "ugert"),
			expectedErr: "invalid denom: 1ugert",
		},
		{
			name:        "invalid token b",
			allowedPool: types.NewAllowedPool("ugert", "1ugert"),
			expectedErr: "invalid denom: 1ugert",
		},
		{
			name:        "matching tokens",
			allowedPool: types.NewAllowedPool("ugert", "ugert"),
			expectedErr: "pool cannot have two tokens of the same type, received 'ugert' and 'ugert'",
		},
		{
			name:        "invalid token order",
			allowedPool: types.NewAllowedPool("usdx", "ugert"),
			expectedErr: "invalid token order: 'ugert' must come before 'usdx'",
		},
		{
			name:        "invalid token order due to capitalization",
			allowedPool: types.NewAllowedPool("ugert", "Ugert"),
			expectedErr: "invalid token order: 'Ugert' must come before 'ugert'",
		},
	}
...
func TestAllowedPool_String(t *testing.T) {
	allowedPool := types.NewAllowedPool("hard", "ugert")
...
func TestAllowedPools_Validate(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedErr: "duplicate pool: hard:ugert",
		},
		{
			name: "duplicate pools",
			allowedPools: types.NewAllowedPools(
