...
import (
	...
	types "github.com/Gert-Chain/core/x/swap/types"
...
func TestState_NewPoolRecord(t *testing.T) {
	reserves := sdk.NewCoins(usdx(50e6), ugert(10e6))
	...
	assert.PanicsWithValue(t, "reserves must have two denominations", func() {
		reserves := sdk.NewCoins(ugert(10e6))
		_ = types.NewPoolRecord(reserves, totalShares)
	}, "expected panic with 1 coin in reserves")

	assert.PanicsWithValue(t, "reserves must have two denominations", func() {
		reserves := sdk.NewCoins(ugert(10e6), hard(1e6), usdx(20e6))
		_ = types.NewPoolRecord(reserves, totalShares)
...
func TestState_NewPoolRecordFromPool(t *testing.T) {
	...
	assert.Equal(t, types.PoolID("ugert", "usdx"), record.PoolID)
	assert.Equal(t, ugert(10e6), record.ReservesA)
	assert.Equal(t, record.ReservesB, usdx(50e6))
	assert.Equal(t, pool.TotalShares(), record.TotalShares)
	assert.Equal(t, sdk.NewCoins(ugert(10e6), usdx(50e6)), record.Reserves())
...
func TestState_PoolRecord_JSONEncoding(t *testing.T) {
	raw := `{
		"pool_id": "ugert:usdx",
		"reserves_a": { "denom": "ugert", "amount": "1000000" },
	...
	assert.Equal(t, types.PoolID("ugert", "usdx"), record.PoolID)
	assert.Equal(t, ugert(1e6), record.ReservesA)
...
total_shares: "3000000"
`
	record := types.NewPoolRecord(sdk.NewCoins(ugert(1e6), usdx(5e6)), i(3e6))
...
func TestState_PoolRecord_Validations(t *testing.T) {
	validRecord := types.NewPoolRecord(
		sdk.NewCoins(usdx(500e6), ugert(100e6)),
		i(300e6),
	)
	testCases := []struct {
		name        string
		...
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ugertusdx' is invalid",
		},
		{
		...
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ugert:' is invalid",
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'usdx:ugert' is invalid",
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ugert:ugert' is invalid",
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ugert:usdx' does not match reserves",
		},
		{
		...
		},
		{
			...
			poolID:      "ugert:usdx",
			reservesA:   sdk.Coin{Denom: "ugert", Amount: sdk.NewInt(-1)},
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ugert:usdx' has invalid reserves: -1ugert",
		},
		{
			name:        "zero reserve a",
			poolID:      "ugert:usdx",
			reservesA:   sdk.Coin{Denom: "ugert", Amount: sdk.ZeroInt()},
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ugert:usdx' has invalid reserves: 0ugert",
		},
		{
			...
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ugert:usdx' has invalid reserves: -1usdx",
		},
		{
			name:        "zero reserve b",
			poolID:      "ugert:usdx",
			...
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ugert:usdx' has invalid reserves: 0usdx",
		},
		{
			...
			expectedErr: "pool 'ugert:usdx' has invalid total shares: -1",
		},
		{
			...
			expectedErr: "pool 'ugert:usdx' has invalid total shares: 0",
		},
	}
...
func TestState_PoolRecord_OrderedReserves(t *testing.T) {
	invalidOrder := types.NewPoolRecord(
		...
		sdk.Coins{usdx(500e6), ugert(100e6)},
		i(300e6),
	)
	...
	validOrder := types.NewPoolRecord(
		// force order to not be sorted
		sdk.Coins{ugert(500e6), usdx(100e6)},
		i(300e6),
	)
	assert.NoError(t, validOrder.Validate())

	record_1 := types.NewPoolRecord(sdk.NewCoins(usdx(500e6), ugert(100e6)), i(300e6))
	record_2 := types.NewPoolRecord(sdk.NewCoins(ugert(100e6), usdx(500e6)), i(300e6))
	// ensure no regresssions in NewCoins ordering
	assert.Equal(t, record_1, record_2)
	assert.Equal(t, types.PoolID("ugert", "usdx"), record_1.PoolID)
	assert.Equal(t, types.PoolID("ugert", "usdx"), record_2.PoolID)
}
...
func TestState_PoolRecords_Validation(t *testing.T) {
	...
	assert.Error(t, err)
	assert.EqualError(t, err, "pool 'ugert:usdx' has invalid total shares: -1")
}
...
func TestState_PoolRecords_ValidateUniquePools(t *testing.T) {
	...
	record_2 := types.NewPoolRecord(
		sdk.NewCoins(usdx(5000e6), ugert(1000e6)),
		i(3000e6),
	)
	...
	assert.EqualError(t, invalidRecords.Validate(), "duplicate poolID 'ugert:usdx'")
}

func TestState_NewShareRecord(t *testing.T) {
	depositor := sdk.AccAddress("some user")
	poolID := types.PoolID("ugert", "usdx")
...
func TestState_ShareRecord_JSONEncoding(t *testing.T) {
	raw := `{
		"depositor": "gert1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w",
		"pool_id": "ugert:usdx",
	...
	assert.Equal(t, "gert1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w", record.Depositor.String())
	assert.Equal(t, types.PoolID("ugert", "usdx"), record.PoolID)
	assert.Equal(t, i(3e6), record.SharesOwned)
}

func TestState_ShareRecord_YamlEncoding(t *testing.T) {
	expected := `depositor: gert1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w
pool_id: ugert:usdx
shares_owned: "3000000"
`
	depositor, err := sdk.AccAddressFromBech32("gert1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w")
	require.NoError(t, err)

	record := types.NewShareRecord(depositor, "ugert:usdx", i(3e6))
	data, err := yaml.Marshal(record)
	require.NoError(t, err)
...
func TestState_ShareRecord_Validations(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedErr: "depositor 'gert1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and pool 'ugert:usdx' has invalid total shares: -1",
		},
		{
			name:        "zero total shares",
			depositor:   validRecord.Depositor,
			...
			expectedErr: "depositor 'gert1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and pool 'ugert:usdx' has invalid total shares: 0",
		},
	}
...
func TestState_ShareRecords_Validation(t *testing.T) {
	...
	err = records.Validate()
	assert.Error(t, err)
	assert.EqualError(t, err, "depositor 'gert1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and pool 'hard:usdx' has invalid total shares: -1")
}

func TestState_ShareRecords_ValidateUniqueShareRecords(t *testing.T) {
	depositor_1, err := sdk.AccAddressFromBech32("gert1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w")
	require.NoError(t, err)

	depositor_2, err := sdk.AccAddressFromBech32("gert1esagqd83rhqdtpy5sxhklaxgn58k2m3s3mnpea")
	require.NoError(t, err)

	record_1 := types.NewShareRecord(depositor_1, "ugert:usdx", i(20e6))
	record_2 := types.NewShareRecord(depositor_1, "ugert:usdx", i(10e6))
	record_3 := types.NewShareRecord(depositor_1, "hard:usdx", i(20e6))
	record_4 := types.NewShareRecord(depositor_2, "ugert:usdx", i(20e6))
	...
	assert.EqualError(t, invalidRecords.Validate(), "duplicate depositor 'gert1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and poolID 'ugert:usdx'")
}
