...
func (suite *GenesisTestSuite) Test_InitExportGenesis() {
	...
	deposits := types.Deposits{
		types.NewDeposit(
			suite.addrs[0],
			sdk.NewCoins(sdk.NewCoin("ugert", sdk.NewInt(1e8))), // 100 ugert
	...
	borrows := types.Borrows{
		types.NewBorrow(
			suite.addrs[1],
			sdk.NewCoins(sdk.NewCoin("ugert", sdk.NewInt(1e7))), // 10 ugert
	...
	accuralTimes := types.GenesisAccumulationTimes{
		types.NewGenesisAccumulationTime("ugert", suite.genTime, supplyInterestFactor, borrowInterestFactor),
	}
