...
import (
	...
	"github.com/Gert-Chain/core/x/issuance/types"
)
...
func GetCmdIssueTokens() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s tx %s issue 20000000usdtoken gert15qdefkmwswysgg4qxgqpqr35k3m49pkx2jdfnw
...
func GetCmdBlockAddress() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s tx %s block gert15qdefkmwswysgg4qxgqpqr35k3m49pkx2jdfnw usdtoken
		`, version.AppName, types.ModuleName),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientTxContext(cmd)
...
func GetCmdUnblockAddress() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s tx %s unblock gert15qdefkmwswysgg4qxgqpqr35k3m49pkx2jdfnw usdtoken
		`, version.AppName, types.ModuleName),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientTxContext(cmd)
