...
Example:
$ %s query %s cdp gert15qdefkmwswysgg4qxgqpqr35k3m49pkx2jdfnw atom-a
...
$ kvcli q cdp cdps --owner=gert1hatdq32u5x4wnxrtv5wzjzmq49sxgjgsj0mffm
...
Example:
$ %s query %s deposits gert15qdefkmwswysgg4qxgqpqr35k3m49pkx2jdfnw atom-a
`, version.AppName, types.ModuleName)),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
