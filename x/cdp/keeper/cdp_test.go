...
func (suite *CdpTestSuite) TestMarketStatus() {
	suite.keeper.SetMarketStatus(suite.ctx, "ugert:usd", true)
	status := suite.keeper.GetMarketStatus(suite.ctx, "ugert:usd")
	suite.Require().True(status)
	suite.keeper.SetMarketStatus(suite.ctx, "ugert:usd", false)
	status = suite.keeper.GetMarketStatus(suite.ctx, "ugert:usd")
