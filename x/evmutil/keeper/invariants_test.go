...
func (suite *invariantTestSuite) SetupValidState() {
	...
	suite.FundModuleAccountWithgert(
		types.ModuleName,
		sdk.NewCoins(
			sdk.NewCoin("ugert", sdk.NewInt(2)), // ( sum of all minor balances ) / conversion multiplier
		),
	)
}
...
func (suite *invariantTestSuite) TestSmallBalances() {
	...
	// add same number of ugert to avoid breaking other invariants
