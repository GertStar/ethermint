...
import (
	...
	"github.com/Gert-Chain/core/x/incentive/keeper"
	"github.com/Gert-Chain/core/x/incentive/testutil"
	"github.com/Gert-Chain/core/x/incentive/types"
)
...
func (suite *BorrowIntegrationTests) TestSingleUserAccumulatesRewardsAfterSyncing() {
	...
	authBulder := app.NewAuthBankGenesisBuilder().
		WithSimpleModuleAccount(gertdisttypes.ModuleName, cs(c("hard", 1e18))). // Fill gertdist with enough coins to pay out any reward
...
func (suite *BorrowRewardsTestSuite) TestAccumulateHardBorrowRewards() {
	...
	testCases := []test{
		...
		},
		{
			...
			args{
				borrow:           c("bnb", 1000000000000),
				rewardsPerSecond: cs(c("hard", 122354), c("ugert", 122354)),
				timeElapsed:      7,
				expectedRewardIndexes: types.RewardIndexes{
					types.NewRewardIndex("hard", d("0.000000856478000001")),
					types.NewRewardIndex("ugert", d("0.000000856478000001")),
				},
			},
		},
		{
			...
			args{
				...
				expectedRewardIndexes: types.RewardIndexes{
					...
					types.NewRewardIndex("ugert", d("0.010571385600010177")),
				},
			},
		},
		{
			...
			args{
				...
				expectedRewardIndexes: types.RewardIndexes{
					...
					types.NewRewardIndex("ugert", d("0.0")),
				},
			},
		},
		{
			...
			args{
				borrow:           c("bnb", 1000000000000),
				rewardsPerSecond: cs(c("hard", 122354), c("ugert", 555555)),
				timeElapsed:      86400,
				expectedRewardIndexes: types.RewardIndexes{
					types.NewRewardIndex("hard", d("0.010571385600010177")),
					types.NewRewardIndex("ugert", d("0.047999952000046210")),
				},
			},
		},
	}
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			authBuilder := app.NewAuthBankGenesisBuilder().WithSimpleAccount(
				userAddr,
				cs(c("bnb", 1e15), c("ugert", 1e15), c("btcb", 1e15), c("xrp", 1e15), c("zzz", 1e15)),
			)
...
func (suite *BorrowRewardsTestSuite) TestSynchronizeHardBorrowReward() {
	...
	testCases := []test{
		...
		},
		{
			...
			args{
				incentiveBorrowRewardDenom: "ugert",
				borrow:                     c("ugert", 1), // borrow a tiny amount so that rewards round to zero
				rewardsPerSecond:           cs(c("hard", 122354)),
				blockTimes:                 []int{10, 10, 10, 10, 10, 10, 10, 10, 10, 10},
		...
		},
		{
			...
			args{
				...
				expectedRewardIndexes: types.RewardIndexes{
					...
					types.NewRewardIndex("ugert", d("0.001223540000173228")),
				},
				expectedRewards: cs(c("hard", 12235400), c("ugert", 12235400)),
			},
		},
		{
			...
			args{
				...
				expectedRewardIndexes: types.RewardIndexes{
					...
					types.NewRewardIndex("ugert", d("10.571385603126235340")),
				},
				expectedRewards: cs(c("hard", 105713856031), c("ugert", 105713856031)),
			},
		},
		{
			...
			args{
				...
				expectedRewardIndexes: types.RewardIndexes{
					types.NewRewardIndex("hard", d("0.001223540000173228")),
					types.NewRewardIndex("ugert", d("0.005555550000786558")),
				},
				expectedRewards: cs(c("hard", 12235400), c("ugert", 55555500)),
			},
		},
		{
			...
			args{
				...
				updatedRewardsPerSecond:    cs(c("hard", 122354), c("ugert", 100000)),
				updatedExpectedRewards:     cs(c("hard", 21142771202), c("ugert", 8640000000)),
				updatedExpectedRewardIndexes: types.RewardIndexes{
					types.NewRewardIndex("hard", d("2.114277120120202320")),
					types.NewRewardIndex("ugert", d("0.864000000049120715")),
				},
		...
		},
		{
			...
			args{
				...
				updateRewardsViaCommmittee: true,
				updatedBaseDenom:           "zzz",
				updatedRewardsPerSecond:    cs(c("hard", 100000), c("ugert", 100500), c("swap", 500)),
				updatedExpectedRewards:     cs(c("hard", 8640000000), c("ugert", 8683200001), c("swap", 43200000)),
				updatedExpectedRewardIndexes: types.RewardIndexes{
					types.NewRewardIndex("hard", d("0.864000000049803065")),
					types.NewRewardIndex("ugert", d("0.868320000050052081")),
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			authBuilder := app.NewAuthBankGenesisBuilder().
				WithSimpleAccount(suite.addrs[2], cs(c("ugert", 1e9))).
				WithSimpleAccount(userAddr, cs(c("bnb", 1e15), c("ugert", 1e15), c("btcb", 1e15), c("xrp", 1e15), c("zzz", 1e15)))
			...
			suite.Require().NoError(
				suite.hardKeeper.Deposit(suite.ctx, suite.addrs[2], cs(c("ugert", 200_000_000))),
			)
			suite.Require().NoError(
				suite.hardKeeper.Borrow(suite.ctx, suite.addrs[2], cs(c("ugert", 100_000_000))),
			)
	...
	testCases := []test{
		{
			...
			args{
				...
				modification:              withdrawModification{coins: cs(c("ugert", 500000000))},
				rewardsPerSecond:          cs(c("hard", 122354)),
				expectedBorrowIndexDenoms: []string{"bnb", "ugert"},
			},
		},
		{
			...
			args{
				...
				modification:              withdrawModification{coins: cs(c("ugert", 500000000), c("bnb", 50000000000), c("xrp", 50000000000))},
				rewardsPerSecond:          cs(c("hard", 122354)),
				expectedBorrowIndexDenoms: []string{"btcb", "ugert", "bnb", "xrp"},
			},
		},
		{
		...
		},
		{
			...
			args{
				initialDeposit:            cs(c("bnb", 1000000000)),
				firstBorrow:               cs(c("bnb", 100000000), c("ugert", 10000000)),
				modification:              withdrawModification{coins: cs(c("bnb", 1100000000)), repay: true},
				rewardsPerSecond:          cs(c("hard", 122354)),
				expectedBorrowIndexDenoms: []string{"ugert"},
			},
		},
		{
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			incentBuilder := testutil.NewIncentiveGenesisBuilder().
				...
				WithSimpleBorrowRewardPeriod("ugert", tc.args.rewardsPerSecond).
...
func (suite *BorrowRewardsTestSuite) TestSimulateHardBorrowRewardSynchronization() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			userAddr := suite.addrs[3]
			authBuilder := app.NewAuthBankGenesisBuilder().WithSimpleAccount(userAddr, cs(c("bnb", 1e15), c("ugert", 1e15), c("btcb", 1e15), c("xrp", 1e15), c("zzz", 1e15)))
