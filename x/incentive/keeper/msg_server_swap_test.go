...
// authBuilder returns a new auth genesis builder with a full gertdist module account.
func (suite *HandlerTestSuite) authBuilder() *app.AuthBankGenesisBuilder {
	return app.NewAuthBankGenesisBuilder().
		WithSimpleModuleAccount(gertdisttypes.ModuleName, cs(c(types.USDXMintingRewardDenom, 1e18), c("hard", 1e18), c("swap", 1e18)))
}
...
func (suite *HandlerTestSuite) TestPayoutSwapClaimMultiDenom() {
	...
	authBulder := suite.authBuilder().
		WithSimpleAccount(userAddr, cs(c("ugert", 1e12), c("busd", 1e12)))

	incentBuilder := suite.incentiveBuilder().
		WithSimpleSwapRewardPeriod("busd:ugert", cs(c("hard", 1e6), c("swap", 1e6)))

	suite.SetupWithGenState(authBulder, incentBuilder)

	// deposit into a swap pool
	suite.NoError(
		suite.DeliverSwapMsgDeposit(userAddr, c("ugert", 1e9), c("busd", 1e9), d("1.0")),
	)
