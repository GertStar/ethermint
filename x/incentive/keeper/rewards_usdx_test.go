...
func (suite *USDXIntegrationTests) TestSingleUserAccumulatesRewardsAfterSyncing() {
	...
	authBulder := app.NewAuthBankGenesisBuilder().
		WithSimpleModuleAccount(gertdisttypes.ModuleName, cs(c(types.USDXMintingRewardDenom, 1e18))). // Fill gertdist with enough coins to pay out any reward
	...
	testCases := []test{
		{
			...
			args{
				ctype:                 "bnb-a",
				rewardsPerSecond:      c("ugert", 122354),
	...
	testCases := []test{
		{
			...
			args{
				...
				expectedRewards:      c("ugert", 12235400),
			},
		},
		{
			...
			args{
				...
				expectedRewards:      c("ugert", 105713856000),
			},
		},
	}
	for _, tc := range testCases {
...
func (suite *USDXRewardsTestSuite) TestSimulateUSDXMintingRewardSynchronization() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.Require().Equal(claim.Reward, sdk.NewCoin("ugert", sdk.ZeroInt()))
