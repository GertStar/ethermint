	...
	tests := testCases{
		{
			...
			args: args{
				accArgs: accountArgs{
					periods: []vestingtypes.Period{
						{Length: 5, Amount: cs(c("ugert", 5))},
						{Length: 5, Amount: cs(c("ugert", 5))},
						{Length: 5, Amount: cs(c("ugert", 5))},
						{Length: 5, Amount: cs(c("ugert", 5))},
					},
					origVestingCoins: cs(c("ugert", 20)),
					startTime:        100,
					endTime:          120,
				},
				period:              vestingtypes.Period{Length: 2, Amount: cs(c("ugert", 6))},
				ctxTime:             time.Unix(101, 0),
				mintModAccountCoins: true,
				expectedPeriods: []vestingtypes.Period{
					{Length: 3, Amount: cs(c("ugert", 6))},
					{Length: 2, Amount: cs(c("ugert", 5))},
					{Length: 5, Amount: cs(c("ugert", 5))},
					{Length: 5, Amount: cs(c("ugert", 5))},
					{Length: 5, Amount: cs(c("ugert", 5))},
				},
		...
		},
		{
			...
			args: args{
				...
				period:              vestingtypes.Period{Length: 7, Amount: cs(c("ugert", 6))},
				ctxTime:             time.Unix(80, 0),
				mintModAccountCoins: true,
				expectedPeriods: []vestingtypes.Period{
					{Length: 7, Amount: cs(c("ugert", 6))},
					{Length: 18, Amount: cs(c("ugert", 5))},
					{Length: 5, Amount: cs(c("ugert", 5))},
					{Length: 5, Amount: cs(c("ugert", 5))},
					{Length: 5, Amount: cs(c("ugert", 5))},
				},
		...
		},
		{
			...
			args: args{
				...
				period:              vestingtypes.Period{Length: 5, Amount: cs(c("ugert", 6))},
				ctxTime:             time.Unix(110, 0),
				mintModAccountCoins: true,
				expectedPeriods: []vestingtypes.Period{
					{Length: 5, Amount: cs(c("ugert", 5))},
					{Length: 5, Amount: cs(c("ugert", 5))},
					{Length: 5, Amount: cs(c("ugert", 11))},
					{Length: 5, Amount: cs(c("ugert", 5))},
				},
				expectedStartTime: 100,
				expectedEndTime:   120,
		...
		},
		{
			...
			args: args{
				...
				period:              vestingtypes.Period{Length: 50, Amount: cs(c("ugert", 6))},
				ctxTime:             time.Unix(110, 0),
				mintModAccountCoins: true,
				expectedPeriods: []vestingtypes.Period{
					{Length: 5, Amount: cs(c("ugert", 5))},
	...
	for _, tc := range tests {
		suite.Run(tc.name, func() {
			...
			if tc.args.mintModAccountCoins {
				authBuilder = authBuilder.WithSimpleModuleAccount(gertdisttypes.ModuleName, tc.args.period.Amount)
			}
			...
			err := suite.keeper.SendTimeLockedCoinsToPeriodicVestingAccount(suite.ctx, gertdisttypes.ModuleName, suite.addrs[0], tc.args.period.Amount, tc.args.period.Length)
...
func (suite *PayoutTestSuite) TestSendCoinsToBaseAccount() {
	authBuilder := app.NewAuthBankGenesisBuilder().
		WithSimpleAccount(suite.addrs[1], cs(c("ugert", 400))).
		WithSimpleModuleAccount(gertdisttypes.ModuleName, cs(c("ugert", 600)))
	...
	err := suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, gertdisttypes.ModuleName, suite.addrs[1], cs(c("ugert", 100)), 5)
	suite.Require().NoError(err)
	acc := suite.getAccount(suite.addrs[1])
	vacc, ok := acc.(*vestingtypes.PeriodicVestingAccount)
	suite.True(ok)
	expectedPeriods := []vestingtypes.Period{
		{Length: int64(5), Amount: cs(c("ugert", 100))},
	}

	bk := suite.app.GetBankKeeper()

	suite.Equal(expectedPeriods, vacc.VestingPeriods)
	suite.Equal(cs(c("ugert", 100)), vacc.OriginalVesting)
	suite.Equal(cs(c("ugert", 500)), bk.GetAllBalances(suite.ctx, vacc.GetAddress()))
...
func (suite *PayoutTestSuite) TestSendCoinsToInvalidAccount() {
	...
	err := suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, gertdisttypes.ModuleName, suite.addrs[2], cs(c("ugert", 100)), 5)
	suite.Require().ErrorIs(err, types.ErrAccountNotFound)

	macc := suite.getModuleAccount(cdptypes.ModuleName)
	err = suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, gertdisttypes.ModuleName, macc.GetAddress(), cs(c("ugert", 100)), 5)
