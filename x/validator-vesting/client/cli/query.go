...
import (
	...
	"github.com/Gert-Chain/core/x/validator-vesting/types"
)

// GetQueryCmd returns the cli query commands for the gertdist module
...
func queryCirculatingSupply() *cobra.Command {
	return &cobra.Command{
		...
		Long:  "Get the current circulating supply of gert tokens",
		Args:  cobra.NoArgs,
...
func queryTotalSupply() *cobra.Command {
	return &cobra.Command{
		...
		Long:  "Get the current total supply of gert tokens",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
