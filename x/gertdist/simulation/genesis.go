...
// RandomizedGenState generates a random GenesisState for gertdist module
func RandomizedGenState(simState *module.SimulationState) {
	...
	gertdistGenesis := types.NewGenesisState(params, types.DefaultPreviousBlockTime)
	if err := gertdistGenesis.Validate(); err != nil {
		panic(err)
	}

	bz, err := json.MarshalIndent(&gertdistGenesis, "", " ")
	if err != nil {
		panic(err)
	}
	fmt.Printf("Selected randomly generated %s parameters:\n%s\n", types.ModuleName, bz)
	simState.GenState[types.ModuleName] = simState.Cdc.MustMarshalJSON(gertdistGenesis)
}
