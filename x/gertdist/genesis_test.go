...
import (
	...
	testutil "github.com/Gert-Chain/core/x/gertdist/testutil"
...
func (suite *genesisTestSuite) TestInitGenesis_ValidationPanic() {
	...
	suite.Require().Panics(func() {
		gertdist.InitGenesis(suite.Ctx, suite.Keeper, suite.AccountKeeper, invalidState)
...
func (suite *genesisTestSuite) TestInitAndExportGenesis() {
	...
	gertdist.InitGenesis(suite.Ctx, suite.Keeper, suite.AccountKeeper, state)
	suite.Require().Equal(state.Params, suite.Keeper.GetParams(suite.Ctx))

	exportedState := gertdist.ExportGenesis(suite.Ctx, suite.Keeper)
